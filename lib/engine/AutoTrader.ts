...
import { getExpiryForSymbol } from '../expiryUtils';
import { strikeStepFor, atmStrike, nearestOtmStrike, getLotSize } from '../contractSpecs';
...
export async function broadcastAlert(signal: Signal) {
  ...
  // Compute expiry & strike via centralised contract spec helpers
  const expInfo = getExpiryForSymbol(signal.symbol);
  const expiryStr = expInfo.display;
  ...
  const tick = strikeStepFor(signal.symbol, price);
  const atm = atmStrike(price, tick);
  const fnoStrike = signal.fnoRecommendation?.strike ?? (
    isCall ? nearestOtmStrike(price, tick, true)
    : isPut ? nearestOtmStrike(price, tick, false)
    : atm
  );

  // Estimate premium
  const dteVal = expInfo.dte;
  const t = dteVal / 365;
  const r = 0.065;
  ...
  const contractLot = getLotSize(signal.symbol) ?? getLotSize(shortName);
  const lotLine = contractLot ? `LOT SIZE: ${contractLot} qty/lot` : null;

  const subject = `🔔 ${tradeCallLine}`;
  const body = hasFnO
    ? `${tradeCallLine}\n\nSTOPLOSS: ${slPrem}\n\nTARGETS: ${t1}-${t2}-${t3}${lotLine ? `\n\n${lotLine}` : ''}\n\nSpot: ₹${price.toFixed(2)}\nStrength: ${signal.strength}% | Confidence: ${(signal.confidence * 100).toFixed(0)}%\n\nSupport/Resistance:\n${signal.reason}\n\nTime: ${signal.timestamp}`
  ...
  const html = `<div style="font-family:monospace;padding:20px;background:#071026;color:#e0e0e0;border-radius:12px;border:2px solid ${sigColor}">
    ...
    ${lotLine ? `<p style="color:#ffd24d;font-weight:900;font-size:14px;margin:8px 0">📦 ${lotLine}</p>` : ''}
