...
export async function GET(req: Request) {
  ...
  if (!broker.getFunds) return NextResponse.json({ provider, funds: { available: 0, currency: "INR" } });
  try {
    const funds = await broker.getFunds(creds);
    return NextResponse.json({ provider, funds });
  } catch (e: any) {
