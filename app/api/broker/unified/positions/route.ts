...
export async function GET(req: Request) {
  ...
  if (!broker.getPositions) return NextResponse.json({ provider, positions: [] });
  try {
    const positions = await broker.getPositions(creds);
    return NextResponse.json({ provider, positions });
  } catch (e: any) {
